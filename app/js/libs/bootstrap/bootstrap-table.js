...
!function ($) {
    ...
    BootstrapTable.prototype.getDataByKey = function (response, keyName) {
        var that = this;
        var s = keyName.split('.');
        var obj = response;
        var hasKey = true;
        if (s.length > 0) {
            for (var i = 0 ; i <= s.length - 1; i++) {
                if (obj.hasOwnProperty(s[i])) {
                    ...
                    that.trigger('load-error', 'json key not found');
                    console.log("json key not found");
                    throw "json key not found";
                }
            }
        }
        else {
    ...
    BootstrapTable.prototype.initServer = function (silent) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                var data;
                if (that.options.datasetProperty == null) {
                    data = res;
                }
                else {
                    data = that.getDataByKey(res, that.options.datasetProperty);
                }
                //if (res.response.model.data)
                //    data = res.response.model.data;
                //else 
